 ...
 */
class ChangeRecyclerViewModeJavaActivity : BaseActivity(), ISearchResult {
    ...
    fun initMaterialRefrshLayoutListener() {
        if (data != null && data!!.size > 0) {
            ...
            recyclerView?.layoutManager = LinearLayoutManager(this@ChangeRecyclerViewModeJavaActivity)
    ...
    private fun addShowStyleChengeListener() {
        cnToolbar!!.setRightImgeButtonIcOnClickListener {
            ...
            Toast.makeText(this@ChangeRecyclerViewModeJavaActivity, "切换格式：$style", Toast.LENGTH_SHORT).show()
            try {
                changeShowType()
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    fun initGridMaterialRefrshLayoutListener() {
        if (data != null && data!!.size > 0) {
            ...
            Toast.makeText(this@ChangeRecyclerViewModeJavaActivity, "该类别暂无商品", Toast.LENGTH_SHORT).show()
        }
    }
