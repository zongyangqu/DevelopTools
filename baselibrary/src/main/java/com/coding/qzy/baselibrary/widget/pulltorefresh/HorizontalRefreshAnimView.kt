...
/**
 * CreateDate:2022/6/17 13:57
 * @author: zongyang qu
 * @Package： com.coding.qzy.baselibrary.widget.pulltorefresh
 * @Description:
 */
class HorizontalRefreshAnimView(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(
    context,
    attrs, defStyleAttr
) {
    ...
    init {
        pullWidth = TypedValue.applyDimension(
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (width > pullDelta + pullWidth) {
            widthMSpec = MeasureSpec.makeMeasureSpec(
                pullDelta + pullWidth,
                MeasureSpec.getMode
                (widthMeasureSpec)
            )
...
//        }
    ...
    private fun drawBack(canvas: Canvas?, delta: Int) {
        with(path) {
            ...
            quadTo(
                delta.toFloat(), (mHeight / 2).toFloat(), (mWidth - pullWidth).toFloat(),
                mHeight.toFloat() - top
            )
...
//        canvas?.drawRect(0f, 0f, mWidth.toFloat(), mHeight.toFloat(), backPaint)
    ...
    private fun drawDrag(canvas: Canvas?) {
        canvas?.drawRect(
            (mWidth - pullWidth).toFloat(), top, mWidth.toFloat(),
            mHeight
                .toFloat() - top,
            backPaint
        )
        ...
        with(path) {
            ...
            quadTo(
                0f, (mHeight / 2).toFloat(), (mWidth - pullWidth).toFloat(),
                mHeight
                    .toFloat() - top
            )
        }
