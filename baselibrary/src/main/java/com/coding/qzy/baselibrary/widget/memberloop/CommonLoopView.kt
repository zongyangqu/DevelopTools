...
/**
 * CreateDate:2021/9/30 16:33
 * @author: zongyang qu
 * @Package： cn.yonghui.hyd.member.wigets.memberloop
 * @Description:
 */
class CommonLoopView @JvmOverloads constructor(
        context: Context,
        attrs: AttributeSet? = null,
        defStyleAttr: Int = 0
) : RelativeLayout(context, attrs, defStyleAttr), ViewPager.OnPageChangeListener, LifecycleObserver {

    private var mViewPager: CommonLoopViewPager? = null
    ...
    private var mLayoutResId= R.layout.member_tk_banner_item
    ...
    init {
        ...
        this.mPointDrawableResId=R.drawable.common_loop_banner_dot
        this.mPointLeftRightMargin = ScreenUtils.dp2px(context, 3)
        this.mPointTopBottomMargin = ScreenUtils.dp2px(context, 6)

        this.mNumberIndicatorTextColor= Color.WHITE
        this.mNumberIndicatorTextSize= ScreenUtils.dp2px(context,14)

        this.mAutoPlayTask = AutoPlayTask(this)

        val typedArray: TypedArray = context.obtainStyledAttributes(attrs, R.styleable.CommonLoopView)
        for (i in 0 until typedArray.indexCount) {
            val attr=typedArray.getIndex(i)
            if (attr == R.styleable.CommonLoopView_banner_autoPlayAble) {
                mAutoPlayAble = typedArray.getBoolean(attr, mAutoPlayAble)
            } else if (attr == R.styleable.CommonLoopView_banner_autoPlayInterval) {
                mAutoPlayInterval = typedArray.getInt(attr, mAutoPlayInterval.toInt()).toLong()
            }else if(attr==R.styleable.CommonLoopView_banner_showIndicator){
                mShowIndicator=typedArray.getBoolean(attr,mShowIndicator)
            }
            else if (attr == R.styleable.CommonLoopView_banner_indicatorGravity) {
                mIndicatorGravity = typedArray.getInt(attr, mIndicatorGravity)
            }
            else if(attr==R.styleable.CommonLoopView_banner_pointContainerLeftRightPadding){
                mPointContainerLeftRightPadding=typedArray.getDimensionPixelSize(attr,0)
            }else if(attr==R.styleable.CommonLoopView_banner_pointContainerTopBottomPadding){
                mPointContainerTopBottomPadding=typedArray.getDimensionPixelSize(attr,0)
            }
            else if (attr == R.styleable.CommonLoopView_banner_pointDrawable) {
                mPointDrawableResId = typedArray.getResourceId(attr,mPointDrawableResId)
            } else if (attr == R.styleable.CommonLoopView_banner_pointLeftRightMargin) {
                mPointLeftRightMargin = typedArray.getDimensionPixelSize(attr, mPointLeftRightMargin)
            } else if (attr == R.styleable.CommonLoopView_banner_pointTopBottomMargin) {
                mPointTopBottomMargin = typedArray.getDimensionPixelSize(attr, mPointTopBottomMargin)
            }else if(attr==R.styleable.CommonLoopView_banner_isNumberIndicator){
                mIsNumberIndicator=typedArray.getBoolean(attr,false)
            }
            else if(attr==R.styleable.CommonLoopView_banner_numberIndicatorTextColor){
                mNumberIndicatorTextColor=typedArray.getColor(attr,this.mNumberIndicatorTextColor)
            }
            else if(attr==R.styleable.CommonLoopView_banner_numberIndicatorTextSize){
                mNumberIndicatorTextSize=typedArray.getDimensionPixelSize(attr,this.mNumberIndicatorTextSize)
            }
            else if(attr==R.styleable.CommonLoopView_banner_numberIndicatorBackground){
    ...
    fun setLifecycleRegistry(lifecycleRegistry: Lifecycle){
    ...
    private fun initViewPager(){
        ...
        mViewPager = CommonLoopViewPager(this.context).apply {
    ...
    fun setData(resIds: IntArray){
        ...
        for (i in resIds.indices) {
            views.add(CommonLoopUtils.getItemImageView(context,resIds[i]))
            models.add(i)
        }
        if(resIds.size<=1){
            mAutoPlayAble=false
        ...
        if(mAutoPlayAble){
            views.add(0,CommonLoopUtils.getItemImageView(context,resIds[resIds.size-1]))
            views.add(CommonLoopUtils.getItemImageView(context,resIds[0]))
        }
        this.setData(
                views=views,
                models = models
    ...
    fun setData(layoutId:Int?=null, models: List<Any?>){
        val views = mutableListOf<View>()
        val layoutResId=layoutId?:R.layout.member_tk_banner_item
        ...
        override fun instantiateItem(container: ViewGroup, position: Int): Any {
            ...
            view.setOnClickListener(object: DebouncingOnClickListener() {
                override fun doClick(v: View) {
                    this@CommonLoopView.mModels!![finalPosition]?.let {
                        (this@CommonLoopView.mOnBannerItemClickListener as OnBannerItemClickListener<Any>?)?.onBannerItemClick(
                                banner = this@CommonLoopView,
                                model = it,
            ...
            this@CommonLoopView.mModels!![finalPosition]?.let {
                (this@CommonLoopView.mAdapter as Adapter<Any>?)?.fillBannerItem(
                        banner = this@CommonLoopView,
                        view = view,
                        model = it,
                        position = finalPosition
     ...
     * 轮播Runnable
     */
    class AutoPlayTask(banner: CommonLoopView) : Runnable {
        private var mBanner: WeakReference<CommonLoopView>? = null

        init {
            mBanner = WeakReference<CommonLoopView>(banner)
        }
