...
class GradientDrawableCreator internal constructor(private val typedArray: TypedArray) : ICreateDrawable {
    ...
    override fun create(): Drawable {
        ...
        if (typedArray.hasValue(R.styleable.background_bl_gradient_startColor) &&
                typedArray.hasValue(R.styleable.background_bl_gradient_endColor) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            val colors: IntArray
        ...
        if (gradientType == GradientDrawable.LINEAR_GRADIENT &&
                typedArray.hasValue(R.styleable.background_bl_gradient_angle) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            gradientAngle %= 360
            if (gradientAngle % 45 != 0) {
                throw XmlPullParserException(typedArray.positionDescription
